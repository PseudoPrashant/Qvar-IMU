...
static const qvar_app_config_t sQvarAppConfig = QVAR_APP_CONFIG_RAW_BOTH;
...
/* Print key interrupt route registers while bringing up QVAR INT2/PC13. */
static void imu_qvar_debug_dump_interrupt_registers(const char *tag)
{
    ...
    printf("[IMU REG] %s IF_CFG=0x%02X CTRL4=0x%02X CTRL7=0x%02X INT1_CTRL=0x%02X INT2_CTRL=0x%02X pin=(PD0:%u,PC13:%u)\r\n",
           (tag != NULL) ? tag : "",
           (unsigned)ifCfg,
           (unsigned)ctrl4,
           (unsigned)ctrl7,
...
HAL_StatusTypeDef imu_configure_qvar_data_ready_interrupt(uint8_t enable)
{
    ...
    /* PC13 is the board connection for ISM330BX INT2. */
    ...
    /* Keep INT2 routed to PC13, not mirrored onto INT1/PD0. */
    ...
    /* Configure PC13 EXTI only when the QVAR interrupt route is enabled. */
    if (enable != 0u)
    {
    ...
    printf("[IMU] Qvar DRDY INT2/PC13 %s%s\r\n",
           (enable != 0u) ? "EN" : "DIS",
           (enable != 0u) ? " latched both-edge" : "");
    return HAL_OK;
}
...
HAL_StatusTypeDef imu_qvar_start(const imu_qvar_config_t *config)
{
    ...
    /* Optionally route AH/QVAR DRDY to INT2/PC13. */
...
HAL_StatusTypeDef imu_qvar_stop(void)
{
    ...
    if ((sQvarStarted != 0u) && (imu_qvar_select(0u, 0u) != HAL_OK))
    {
        return HAL_ERROR;
    }

    (void)imu_configure_qvar_data_ready_interrupt(0u);
    sQvarStarted = 0u;
    imu_internal_qvar_restore_motion_if_needed();
    return HAL_OK;
}
...
HAL_StatusTypeDef imu_qvar_read_raw(imu_qvar_raw_t *raw)
{
    ...
    if (sQvarConfig.enableQvar1 != 0u)
    {
        ...
        if (status == HAL_OK)
        {
            HAL_Delay(sQvarConfig.settlingMs);
...
void imu_qvar_app_task(void)
{
    ...
    if (sImuAppQvarStarted == 0u)
    {
        ...
        if (imu_qvar_start(&config) == HAL_OK)
        {
            ...
            sImuAppQvarStartMs = HAL_GetTick();
